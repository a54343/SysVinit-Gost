...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    wget --no-check-certificate https://gotunnel.oss-cn-shenzhen.aliyuncs.com/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  else
    rm -rf gost-linux-"$bit"-"$ct_new_ver".gz
    ...
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    wget --no-check-certificate https://raw.githubusercontent.com/KANIKIG/Multi-EasyGost/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  fi
...
# Default-Stop:      0 1 6
# Short-Description: Start gost at boot time
# Description:       Enable gost service
### END INIT INFO

DAEMON=/usr/bin/gost
CONF=/etc/gost/config.json
NAME=gost
...
case "\$1" in
    start)
        echo "Starting \$NAME"
        start-stop-daemon --start --background --make-pidfile --pidfile \$PIDFILE --exec \$DAEMON -- -C \$CONF
        ;;
    stop)
        echo "Stopping \$NAME"
...
EOF
  ...
  echo "------------------------------"
  if test -a /usr/bin/gost -a /etc/init.d/gost -a /etc/gost/config.json; then
    echo "gost安装成功"
    rm -rf "$(pwd)"/gost
    rm -rf "$(pwd)"/gost.service
    rm -rf "$(pwd)"/config.json
...
function cron_restart() {
  ...
  if [ "$numcron" == "1" ]; then
    ...
    if [ "$numcrontype" == "1" ]; then
      ...
      echo "0 0 */$cronhr * * ? * /etc/init.d/gost restart" >> /etc/crontab
      echo -e "定时重启设置成功！"
...
echo && echo -e "                 gost 一键安装配置脚本"${Red_font_prefix}[${shell_version}]${Font_color_suffix}"
  ----------- KANIKIG -----------
  特性: (1)本脚本采用SysVinit及gost配置文件对gost进行管理
