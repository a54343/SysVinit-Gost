...
check_status() {
    ...
    if service V2bX status | grep -q running; then
        return 0
    else
        return 1
    fi
...
# Default-Stop:      0 1 6
# Short-Description: V2bX service
# Description:       V2bX service script
### END INIT INFO

DAEMON=/usr/local/V2bX/V2bX
PIDFILE=/var/run/V2bX.pid
CONFIG=/etc/V2bX/config.json

start() {
    echo "Starting V2bX..."
    start-stop-daemon --start --background --pidfile \$PIDFILE --make-pidfile --exec \$DAEMON -- -config \$CONFIG
}
...
case "\$1" in
    ...
    status)
        if [ -f \$PIDFILE ] && kill -0 \$(cat \$PIDFILE) 2>/dev/null; then
            echo "V2bX is running"
        else
            echo "V2bX is not running"
        fi
        ;;
    *)
...
EOF
    ...
    update-rc.d V2bX defaults
   
   # 停止可能正在运行的服务
    if [ -f /var/run/V2bX.pid ]; then
    ...
    # 检查服务状态
    if /etc/init.d/V2bX status | grep -q "is running"; then
        echo -e "${green}V2bX 启动成功${plain}"
    else
        echo -e "${red}V2bX 可能启动失败，请使用 /etc/init.d/V2bX status 查看状态${plain}"
    fi
